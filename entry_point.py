...
from orthanc_ext.scripts.hello import hello_world
from orthanc_ext.scripts.study_new_to_rest import study_new_to_rest
...
event_dispatcher.register_event_handlers({
    orthanc.ChangeType.ORTHANC_STARTED: [
        partial(log_event, 'Orthanc Started - logged by python script'),
        partial(create_stream, nats_config), start_maintenance_cycle,
        pipeline(retrieve_system_info, show_system_info),
        pipeline(retrieve_system_info, hello_world)
    ],
    orthanc.ChangeType.NEW_STUDY: [
        partial(log_event, 'New Study Acquired - logged by python script'),
        study_new_to_rest       
    ],
